    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        for i in inPlay{
            ...
            if ((xCord > enemyGoal1x!) && (xCord < enemyGoal2x!) && (yCord > enemyGoaly!)){
                ...
                self.onlineScore.fontSize = self.frame.width / 12
                //self.addChild(highScoreLabel!)
                self.endLabel.isHidden = false
                self.again.isHidden = false
              //  self.onlineScore.text = "Reviewing your score"
                self.onlineScore.isHidden = false
                    if (score < 5){
                        self.onlineScore.text = "Just another no man sky"
                    }
                    else if (score > 25){
                        self.onlineScore.text = "10/10 -IGN"
                    }
                    else{
                        self.onlineScore.text = "a typical Call of Duty Game"
                    }
                self.gameOver = true
            }
            }
